...
def test_asset(symbol):
    ...
    try:
        ...
        results = backtest(df, starting_cash=1000.0, fee_rate=0.001,
                            trailing_stop_col="ATR_pct", atr_multiplier=ATR_MULTIPLIER)

        return {
            "symbol": symbol,
            "avg_daily_volatility_pct": round(avg_atr_pct, 2),
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 85)
    print(f"{'Asset':<10}{'AvgVol%':>10}{'Strategy':>12}{'Buy&Hold':>12}{'Trades':>10}{'Beat B&H?':>14}")
    print("=" * 85)
    for r in all_results:
        if "error" in r:
            ...
            print(f"{r['symbol']:<10}{r['avg_daily_volatility_pct']:>9.2f}%"
                  f"{r['strategy_return']:>11.2f}%"
                  f"{r['buy_hold_return']:>11.2f}%{r['num_trades']:>10}{beat:>14}")

    pd.DataFrame(all_results).to_csv("multi_asset_atr_trailing_results.csv", index=False)
    print("\nSaved full results to multi_asset_atr_trailing_results.csv")